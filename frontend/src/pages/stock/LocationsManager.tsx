...
import { Plus, Loader2, ChevronRight, Truck, Factory, ClipboardCheck, Search, X, Pencil, Check, Archive } from "lucide-react";
...
function LocationDetail({
  ...
  async function deactivate() {
    try {
      ...
      onToast({ message: `${location.name} deactivated.`, type: "success" });
      onChanged();
      onBack();
    } catch (ex) {
      onToast({ message: ex instanceof Error ? ex.message : "Failed to deactivate", type: "error" });
    }
  }
    ...
    <div className="space-y-4">
      ...
      {editing && (
        <div className={`flex flex-wrap items-end gap-3 px-4 py-3 ${cardShell}`}>
          ...
          {location.type === "van" && <button type="button" onClick={() => void deactivate()} className="inline-flex items-center gap-1.5 rounded-md border border-[#E6C9C9] bg-white px-3 py-2 text-[13px] font-semibold text-[#C44545] hover:bg-[#FBE5E5]"><Archive className="h-3.5 w-3.5" /> Deactivate</button>}
        </div>
      )}
