...
export function JobBoxSection({ jobId, jobKind, canManage, profiles, defaultAssignee, jobTitle, jobAddress, onChanged }: Props) {
  ...
  return (
    <section>
        ...
        {canManage && (
          <div className="flex items-center gap-1.5">
            <button
              ...
              disabled={siteBusy}
              className="inline-flex items-center gap-1 rounded-full border border-[#E6E1D4] px-2.5 py-1 text-[11px] font-semibold text-[#3A3A3A] hover:border-[#2F8F5C] hover:text-[#246F47] disabled:opacity-50"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-1 rounded-full border border-[#E6E1D4] px-2.5 py-1 text-[11px] font-semibold text-[#3A3A3A] hover:border-[#2F8F5C] hover:text-[#246F47]"
            >
      ...
      {error && <p className="mb-2 text-[11px] text-[#C44545]">{error}</p>}
      {siteNotice && <p className="mb-2 rounded bg-[#E1F3EA] px-2 py-1 text-[11px] text-[#2F8F5C]">{siteNotice}</p>}
      ...
      {loading ? (
        ...
        <ul className="space-y-2">
          {allocations.map((a) => {
            ...
            return (
              <li key={a.id} className="rounded-lg border border-[#E6E1D4] px-3 py-2.5">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <Package className="h-4 w-4 shrink-0 text-[#A0A0A0]" />
                  ...
                  <div className="mt-1.5 flex justify-end">
                    {confirmCancelId === a.id ? (
                      <span className="inline-flex items-center gap-2 text-[11px]">
                        <span className="text-[#6B6B6B]">Cancel this box?</span>
                        <button
                          type="button"
                          onClick={() => void handleCancel(a.id)}
                          disabled={cancellingId === a.id}
                          className="font-semibold text-[#C44545] hover:text-[#A83838] disabled:opacity-50"
                        >
                          {cancellingId === a.id ? 'Cancelling…' : 'Yes, cancel'}
                        </button>
                        <button type="button" onClick={() => setConfirmCancelId(null)} className="text-[#6B6B6B] hover:text-[#3A3A3A]">
                          Keep
                        </button>
                      </span>
                    ) : (
                      <button
                        ...
                        className="inline-flex items-center gap-1 text-[11px] font-medium text-[#A0A0A0] hover:text-[#C44545]"
                      >
      ...
      {usage && usage.lines.length > 0 && (
        <div className="mt-3 rounded-lg border border-[#E6E1D4] bg-[#FAF8F2] px-3 py-2.5">
          <p className="mb-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#6B6B6B]">
          ...
          <ul className="space-y-0.5">
            {usage.lines.map((l, i) => {
              ...
              return (
                <li key={i} className="flex items-center justify-between text-xs text-[#6B6B6B]">
          ...
          <p className="mt-1.5 flex items-center justify-between border-t border-[#E6E1D4] pt-1.5 text-xs font-semibold text-[#1A1A1A]">
...
function AllocateStockModal({ jobId, jobKind, profiles, defaultAssignee, onClose, onCreated }: {
  ...
  // Busy-guard: backdrop/Esc/X all no-op while the create is in flight, so a
  // stray click can't dismiss the modal mid-write (exemplar: MaterialsTab).
  ...
  return (
    <MotionDrawer
      ...
      variant="modal"
      ariaLabel="Allocate stock — pack a job box"
      sizeClass="sm:max-w-lg"
    >
      <div className="flex-1 overflow-y-auto p-5">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-[#1A1A1A]">Allocate stock — pack a job box</h3>
          <button type="button" onClick={requestClose} disabled={saving} className="grid min-h-11 min-w-11 place-items-center rounded-md text-[#A0A0A0] hover:bg-[#F0EDE4] hover:text-[#3A3A3A] disabled:opacity-50">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-[11px] font-semibold uppercase tracking-[0.14em] text-[#6B6B6B]">From</label>
              <select
                value={sourceId}
            ...
            <div>
              <label className="mb-1 block text-[11px] font-semibold uppercase tracking-[0.14em] text-[#6B6B6B]">For (accepts at pickup)</label>
              <select
                value={assignee}
          ...
          <div>
            <label className="mb-1 block text-[11px] font-semibold uppercase tracking-[0.14em] text-[#6B6B6B]">Add items</label>
            <div className="relative">
              <Search className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[#A0A0A0]" />
              <input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder={levelsLoading ? 'Loading stock…' : 'Search this location’s stock (name or SKU)…'}
                disabled={levelsLoading}
                className={cn(inputField, 'pl-9')}
              />
            </div>
            {results.length > 0 && (
              <ul className="mt-1 max-h-44 overflow-y-auto rounded-md border border-[#E6E1D4]">
                {results.map((lvl) => (
                  <li key={lvl.id}>
                    <button
                      type="button"
                      onClick={() => addLine(lvl)}
                      className="flex w-full items-center justify-between px-3 py-2 text-left text-sm hover:bg-[#E1F3EA]"
                    >
                      <span className="truncate text-[#1A1A1A]">{lvl.name}{lvl.sku ? <span className="ml-1.5 font-mono text-[11px] text-[#A0A0A0]">{lvl.sku}</span> : null}</span>
                      <span className={`ml-2 shrink-0 text-xs tabular-nums ${lvl.qty <= 0 ? 'text-[#C44545]' : 'text-[#6B6B6B]'}`}>
                        {lvl.qty} on hand
                      </span>
                    </button>
                  </li>
          ...
          {lines.length > 0 && (
            <ul className="space-y-1.5">
              {lines.map((l) => {
                ...
                return (
                  <li key={l.materialId} className="rounded-md border border-[#E6E1D4] px-3 py-2">
                    <div className="flex items-center justify-between gap-2">
                      <span className="min-w-0 truncate text-sm text-[#1A1A1A]">{l.name}</span>
                      <div className="flex shrink-0 items-center gap-1.5">
                        <input
                          ...
                          className={cn(inputField, 'h-9 w-20 px-2 text-right tabular-nums')}
                        />
                        <span className="w-6 text-xs text-[#A0A0A0]">{l.unit}</span>
                        <button
                          type="button"
                          onClick={() => removeLine(l.materialId)}
                          className="rounded p-1.5 text-[#A0A0A0] hover:text-[#C44545]"
          ...
          <div>
            <label className="mb-1 block text-[11px] font-semibold uppercase tracking-[0.14em] text-[#6B6B6B]">Note (optional)</label>
            <input
              value={note}
          ...
          {error && <p className="text-[11px] text-[#C44545]">{error}</p>}
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex gap-2">
              <button
                ...
                onClick={onClose}
                disabled={saving}
                className="rounded-full border border-[#E6E1D4] px-4 py-2 text-xs font-semibold text-[#3A3A3A] hover:border-[#D8D2C4] disabled:opacity-50"
              >
                Cancel
              </button>
              <button
