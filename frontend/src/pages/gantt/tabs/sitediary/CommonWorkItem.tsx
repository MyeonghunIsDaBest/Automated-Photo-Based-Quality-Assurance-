...
//
// Single tile inside the common-works grid. Click adds the template name
// to the (future) entry composer; v1 is decoration.

import { Plus } from 'lucide-react';
import type { MockCommonWorkItem } from './mockTimeline';

interface CommonWorkItemProps {
  item: MockCommonWorkItem;
}

const ICON_BG: Record<MockCommonWorkItem['category'], string> = {
...
export function CommonWorkItem({ item }: CommonWorkItemProps) {
  return (
    <button
      type="button"
    ...
    >
      ...
      <span className="flex-1 min-w-0">
        ...
        <span className="block text-[10.5px] text-[#A0A0A0] mt-0.5">
          {item.isFrequent ? (
            <><span className="text-[#C8841E]">●</span> Used {item.usageCount}× this week</>
          ) : (
            <>{item.usageCount}×</>
          )}
        </span>
