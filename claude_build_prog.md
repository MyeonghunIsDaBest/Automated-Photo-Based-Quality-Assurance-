...
## 25 June 2026 — Quote rework Phase 1 Part 10: the Customer Assets tab (quote tabs COMPLETE)

Built the **Customer Assets** tab — the 4th and last top-level quote tab. With it, the quote editor now fully matches Simpro's tab shell: **Details / Parts & Labour / Schedule / Customer Assets**. Built with the frontend-design skill for container consistency with the Schedule tab + ledger system. Subagent CI-killer review clean (supabase upsert signature verified against existing usage; `editing` union narrowing sound; no tests affected).

- **Migration 86** (`86_customer_assets.sql`, force-added, **Jordan applies**): `customer_assets` (customer_id fk, property_id, name, asset_type, make, model, serial, location, install_date, warranty_until, notes, is_active…) + touch trigger + index; `quote_assets` link (quote_id, asset_id, unique(quote_id,asset_id)) + indexes. Both manager RLS. **Next free migration = 87.**
- **customerAssets.ts** (NEW): `CustomerAsset` + `listCustomerAssets`/`createAsset`/`updateAsset`/`deleteAsset` + `listQuoteAssetIds`/`attachAsset`(idempotent upsert)/`detachAsset`.
- **QuoteCustomerAssets.tsx** (NEW): a register of the customer's serviceable equipment as a responsive card grid (`sm:2 / xl:3`, consistent ledger cards). Each card: name + type pill, make/model/serial/location/installed/warranty, notes; an **Attach/Attached** toggle that links the asset to this quote (`quote_assets`, optimistic) with an emerald ring + "On this quote" pill when attached; edit + delete (manager register management). Inline **Add/Edit asset** form (name req + type datalist of Casone asset types + make/model/serial/location/install/warranty/notes). Header shows "{n} on this quote · {m} total". States: no-customer ("attach a customer on Details"), empty, loading, error; read-only when locked; `print:hidden`.
